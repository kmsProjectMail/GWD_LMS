...
<input type="text" value="${vo.alarm_date}">
<form name=exf1>
...
<form name=exf1>
		<c:set var = "str" value = "${gg.alarmdate}"/>
		 <c:choose>
			<c:when test="${fn:substring(str, 11,13) eq 09}">
				<input type="hidden" name=h id="h" value="9">
			</c:when>
		...
		<input type="hidden" name=s id="s" value="0" >
		<!-- <input type=button name=b onclick="setAlarm()" value="Set Alarm">
		<input type=button name=r onclick="clearAlarm()" value="Turn Alarm Off"><BR> -->
		<input type=text name=ch size=2>
		<input type=text name=cm size=2> 
		<input type=text name=cs size=2>
		<input type="hidden" value="${gg.id}" id="userid">
